...
- [x] A story URL checked in a link-preview debugger — title, description and image all present,
      confirmed on production 2026-08-26. ⚠️ **Both shapes were checked, and the second is the one
      that mattered**: `/news.html?story=2026-08-14-0`, the OLD link form that exists in anything
      shared before the merge. A scraper has to follow the 301 and render the story rather than a
      redirect notice — which is exactly what `middleware.js` used to fake, and what server
      rendering replaced. `og:image` is the site-wide cover, 1200×630, identical on every story
...
- [x] ⚠️ **The two-account RLS proof** — done on production 2026-08-26, all five probes refused.
      The decisive one is probe 4: `403`, *"new row violates row-level security policy for table
