...
| | |
|---|---|
| **Colour** | The tokens in [public/styles.css](../public/styles.css) — deep teal `#2D756F`, navy `#1F4D4A`, sage `#ACC4B6`, off-white `#EEF2EF`, terracotta `#8A4B2C`, moss `#55643A`. Use the semantic layer (`--bg-*`, `--fg-*`, `--line-*`), not raw hexes |
| **Type** | **Poppins** for headings and UI labels, **Inter** for body and data. Source Serif 4 exists but is reserved for *editorial* headlines — section cards, News articles, skill primer/plan headlines. A dashboard heading is not editorial. ⚠️ The 2026-08-20 mockup gets this wrong and uses the serif; do not copy it |
| **No new typefaces, and no new font host** | [privacy.html](../public/privacy.html) names every outbound third-party request. Adding a face or a host makes that page *wrong*, not out of date, and it must change in the same commit |
...
defects: it uses the serif face where Poppins belongs, and it only draws the "typical" state.
