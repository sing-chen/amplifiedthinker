...
const res = await fetch(
  `${url}/rest/v1/news_stories?select=slug,legacy_id,story_date,title,url&status=eq.published&limit=2000`,
...
console.log(`\n${target} — ${dbRows.length} published stories in the database, ${fileRows.length} in content/news.json\n`);
...
const dbByLegacy = new Map(dbRows.map((r) => [r.legacy_id, r]));
...
const onlyDb = dbRows.filter((r) => !fileLegacy.has(r.legacy_id));
if (onlyDb.length) {
  warnings++;
